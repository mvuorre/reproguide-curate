...
In this tutorial we have already created a local project, and made and committed some changes in it. To make this project collaborative, we need to link this local repository to a **remote** repository, such as one hosted on GitHub. To do this, we must first create a repository on GitHub.
...
You can connect the existing local repository to the newly created GitHub remote with two short lines of commands in the computer's command line---we show how in the supplemental materials. However, you cannot link the existing local repository with the GitHub remote with R Studio. If you do not want to write out the commands in the command line, you can circumvent this problem by creating a new project in R Studio as you did above. Instead of choosing "New Directory" in Figure \@ref(fig:rstudio-project-1), you create a new local project from the GitHub remote (Figure \@ref(fig:clone-from-github)). Although this workflow avoids the use of command line functions, and can be used, it is not recommended because you will have to manually copy-paste the project's contents from the old local folder to the new one. This copy-pasting will work, but by doing so you will leave the old local repository behind, and therefore lose the project's history. However, this method is entirely appropriate for new projects.

After you have created the new remote repository on GitHub, you need the .git repository's URL, which you need to use when setting up the new project in R Studio. This is visible on GitHub on the page that appears after creating a new repository. Copy-paste the repository's URL from the address box on GitHub (e.g. https://github.com/username/reponame.git). Then, in R Studio, click "File" -> "New Project", and select "Version Control" (left panel of Figure \@ref(fig:clone-from-github)). This option allows you to create a new local repository by checking out a GitHub remote repository. Click Git (Figure \@ref(fig:clone-from-github), middle panel), then copy-paste the GitHub URL to the "Repository URL" box, and choose an appropriate location for the project ("Documents/" in Figure \@ref(fig:clone-from-github), right panel). 

Because you cannot associate an existing local Git repository with GitHub by using R Studio, you've now associated the empty remote (GitHub) repository with a local folder on your computer. Because we've already created files and folders in our tutorial project, we can now simply copy-paste the materials from the old project folder to the new empty folder, which is connected to GitHub. After you have copy-pasted the materials to the new local folder make sure that the contents of the old `.gitignore` file are also copied to the new file. Otherwise you will be committing files into Git's history that you would rather ignore. You can then commit all the files using the steps detailed above, after which you will have two new buttons in R Studio's Git panel: The local repository can now send (**push**) and receive (**pull**) changes from the remote repository. Click "Push", and then navigate to the repository's GitHub page and refresh the page: You will see all the committed files and folders on GitHub. We also detail how to do these steps without losing the repository's history by using the command line tools in the supplemental file.
